...
/// Returns true if rng1 elements are equivalent to elements at prefix of rng2
/// by given equivalence relationship.
///
/// # Precondition
///   - bi_pred should follow equivalence relationship.
///
/// # Postcondition
///   - Returns true if rng1 is equivalent to prefix of rng2 by relationship
///     bi_pred.
///   - Complexity: O(n). Maximum `n` `bi_pred` applications.
///
...
/// let arr2 = [1, 2, 3, 4, 5];
///
/// let is_eq = rng::equals_prefix_by(&arr1, &arr2, |x, y| x == y);
/// assert!(is_eq);
///
/// let is_eq = arr1.equals_prefix_by(&arr2, |x, y| x == y);
/// assert!(is_eq);
/// ```
pub fn equals_prefix_by<R1, R2, Binaryred>(
    rng1: &R1,
    rng2: &R2,
    bi_pred: Binaryred,
) -> bool
...
/// Returns true if rng1 elements are equal to elements at prefix of rng2.
///
/// # Precondition
///
/// # Postcondition
...
/// assert!(is_eq);
///
/// let is_eq = arr1.equals_prefix(&arr2);
/// assert!(is_eq);
/// ```
